...
# ============================================================
# MEAN REVERSION EA – TICK‑BASED + IMMEDIATE TRADING + ERROR 4756 FIX
# ============================================================
...
//+------------------------------------------------------------------+
//| Exit when Z returns to mean and profit >0                        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| OPEN ORDER – WITH ERROR 4756 RETRY (FIXED)                       |
//+------------------------------------------------------------------+
void OpenOrder(ENUM_ORDER_TYPE type, double zScore, double mean, double stddev)
{
   ...
   // --- Filling mode: for market execution force FOK (most compatible) ---
   ENUM_ORDER_TYPE_FILLING filling = ORDER_FILLING_RETURN;
   int fillMode = (int)SymbolInfoInteger(symbol, SYMBOL_FILLING_MODE);
   if(isMarketExe)
   {
      filling = ORDER_FILLING_FOK;   // many market brokers require FOK
   }
   else
   {
   ...
   // --- Send order with retry on error 4756 ---
   if(!OrderSend(req, res))
   {
      int error = GetLastError();
      Print("OrderSend failed | error=", error);
      if(error == 4756 && isMarketExe)
      {
         // Retry with RETURN filling mode
         req.type_filling = ORDER_FILLING_RETURN;
         if(OrderSend(req, res))
         {
            if(res.retcode == TRADE_RETCODE_DONE || res.retcode == TRADE_RETCODE_PLACED || res.retcode == 0)
               Print("Retry successful (RETURN) | ", (type==ORDER_TYPE_BUY?"BUY":"SELL"));
            else
               Print("Retry failed | retcode=", res.retcode);
         }
         else
